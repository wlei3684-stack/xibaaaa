...
from tracking_data.upstream.utils import TensorDict
import numpy as np
...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap, num_search_frames, num_template_frames=1, processing=no_processing, frame_sample_mode='causal', train_cls=False, pos_prob=0.5):
        """
        args:
    ...
    def getitem(self):
        ...
        while not valid:
            _retry_count_2 += 1
            if _retry_count_2 > getattr(self, 'max_attempts', 20):
                raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        _retry_count_1 += 1
                        if _retry_count_1 > getattr(self, 'max_attempts', 20):
                            raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
                        ...
                        search_frame_ids = self._sample_visible_ids(visible, min_id=template_frame_ids[0] + 1, max_id=template_frame_ids[0] + self.max_gap + gap_increase, num_ids=self.num_search_frames)
                        gap_increase += 5
            ...
            try:
                ...
                warnings.warn(f"OSTrack sample failed: {error}", stacklevel=2)
                valid = False
        return data
    ...
    def getitem_cls(self):
        ...
        while not valid:
            _retry_count_3 += 1
            if _retry_count_3 > getattr(self, 'max_attempts', 20):
                raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
            dataset = random.choices(self.datasets, self.p_datasets)[0]
            is_video_dataset = dataset.is_video_sequence()
            (seq_id, visible, seq_info_dict) = self.sample_seq_from_dataset(dataset, is_video_dataset)
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            _retry_count_4 += 1
            if _retry_count_4 > getattr(self, 'max_attempts', 20):
                raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
            seq_id = random.randint(0, dataset.get_num_sequences() - 1)
            seq_info_dict = dataset.get_sequence_info(seq_id)
            visible = seq_info_dict['visible']
            enough_visible_frames = visible.type(torch.int64).sum().item() > 2 * (self.num_search_frames + self.num_template_frames) and len(visible) >= 20
    ...
    def get_frame_ids_trident(self, visible):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            _retry_count_5 += 1
            if _retry_count_5 > getattr(self, 'max_attempts', 20):
                raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
            template_frame_ids_extra = []
    ...
    def get_frame_ids_stark(self, visible, valid):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            _retry_count_6 += 1
            if _retry_count_6 > getattr(self, 'max_attempts', 20):
                raise RuntimeError('OSTrack sampling retry limit reached: check dataset paths, visible frames and annotations')
            template_frame_ids_extra = []
            template_frame_id1 = self._sample_visible_ids(visible, num_ids=1)
            search_frame_ids = self._sample_visible_ids(visible, num_ids=1)
