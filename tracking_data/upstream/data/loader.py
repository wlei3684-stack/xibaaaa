...
from tracking_data.upstream.utils import TensorDict, TensorList
string_classes = (str,)
int_classes = int
...
def ltr_collate(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if _check_use_shared_memory():
            ...
            out = batch[0].new_empty((len(batch), *batch[0].shape)).share_memory_()
...
def ltr_collate_stack1(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if _check_use_shared_memory():
            ...
            # shared memory tensor to avoid an extra copy
            numel = sum([x.numel() for x in batch])
            out = batch[0].new_empty((*batch[0].shape[:1], len(batch), *batch[0].shape[1:])).share_memory_()
